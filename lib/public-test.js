  ...
  sw.on('listening', function () {
    ...
    sw.on('connection', function (connection) {
      ...
      connection.on('data', function (remote) {
        dataEcho = true
        state.title = `Connected to Dat Doctor via ${opts.tcp ? 'TCP' : 'UDP'}`
        destroy(cb)
      })
    ...
    var destroy = thunky(function (cb) {
      sw.destroy(function () {
        if (connected && dataEcho) return cb()
        state.title = `Public Peer Test via ${opts.tcp ? 'TCP' : 'UDP'} Failed`
        if (!connected) {
          cb('Unable to connect to Dat public server')
        }
        if (!dataEcho) {
          cb('Data was not echoed back from public server')
        }
        cb()
      })
    })
